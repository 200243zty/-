...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService{
    ...
    public Result addComment(Comment comment) {
        ...
        if (i > 0) {
            return Result.ok("评论成功",comment.getId().toString());
        } else {
            return Result.ok("评论失败",comment.getId().toString());
        }
    }
     ...
     */
    @Override
    public List<Comment> getAllCommentBySongListId(Long id) {
        //首先查询所有歌单的一级评论
        LambdaQueryWrapper<Comment> lqw =new LambdaQueryWrapper<>();
        lqw.eq(Comment::getSongListId, id);
...
//        return Result.ok("查询成功", allCommentBySongId);
    ...
    public Result getAllCommentByUserId(Long id) {
        ...
        List<Comment> comments = commentMapper.selectList(lqw);
        return Result.ok("获取指定用户所有评论成", comments);
