...
//    }
    ...
    public Result getSongListByPage(Integer currentPage) {
        ...
        LambdaQueryWrapper<SongList> lqw = new LambdaQueryWrapper<>();
        //公开歌单可以查询
        lqw.eq(SongList::getIsPrivate, 0);
        IPage<SongList> songListIPage = songListMapper.selectPage(page, lqw);
        List<SongList> records = songListIPage.getRecords();
        if (!records.isEmpty()) {
            return Result.ok("查询成功", records);
...
//        songListMapper
     ...
     */
    @Autowired
    private SongListStyleMapper songListStyleMapper;

    @Override
    public Result getSongListByStyleAndPage(String style, Integer currentPage) {
        LambdaQueryWrapper<SongListStyle> lqw2 =new LambdaQueryWrapper<>();
        lqw2.eq(SongListStyle::getStyle, style);
        SongListStyle songListStyle = songListStyleMapper.selectOne(lqw2);

        IPage<SongList> page = new Page<>(currentPage, 10);
        LambdaQueryWrapper<SongList> lqw = new LambdaQueryWrapper<>();
        lqw.like(Objects.nonNull(style), SongList::getStyleId, songListStyle.getId());
        IPage<SongList> songListIPage = songListMapper.selectPage(page, lqw);
         List<SongList> records = songListIPage.getRecords();
        if (!records.isEmpty()) {
            return Result.ok("查询成功", records);
     ...
     */
    @Override
    public Result getAllSongListCountByStyle(String name) {
        Integer allSongListCountByStyle = songListMapper.getAllSongListCountByStyle(name);
        return Result.ok("查询成功", allSongListCountByStyle);
    }

    @Override
    public Result getSongListByCreaterId(String createrId) {
        LambdaQueryWrapper<SongList> lqw =new LambdaQueryWrapper<>();
        lqw.eq(SongList::getCreaterId, createrId);
        List<SongList> songLists = songListMapper.selectList(lqw);
        return Result.ok("查询我创建的歌单成功", songLists);
    }
